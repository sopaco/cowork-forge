...
| 容器 | 职责 | 主要依赖 |
|------|------|----------|
| **cowork-core** | 域模型、流水线编排、Agent/工具、持久化、LLM 适配 | adk-rust 生态、Tokio、serde |
...
- **Pipeline**：`Stage` trait 实现 + `StageExecutor` + `PipelineExecutor` 编排
...
2. `PipelineExecutor` 按 flow 配置依次执行 7 阶段（`default.json`）
...
| LLM 限流 | `crates/cowork-core/src/llm/rate_limiter.rs` | 信号量 + 延迟 |