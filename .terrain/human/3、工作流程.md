...
```mermaid
flowchart TB
    subgraph Pipeline["7 阶段流程"]
        direction TB
        S1[创意] --> S2[PRD] --> S3[设计] --> S4[计划] --> S5[编码] --> S6[检查] --> S7[交付]
    end
    
    subgraph Pattern["每阶段 Actor-Critic 模式"]
        Actor[AI Actor<br/>生成工件] --> Artifact[阶段工件]
        Critic[AI Critic<br/>验证质量] -->|反馈| Actor
        Artifact --> HITL{HITL 门}
        HITL -->|通过| NextStage
        HITL -->|反馈| Actor
        HITL -->|编辑| ExternalEditor
    end
...
sequenceDiagram
    ...
    CLI->>PC: 初始化 PipelineContext
    PC->>Store: 创建初始迭代（草稿状态）
...
flowchart TD
    ...
    B -->|架构, 重构, 重新设计| C[完整继承<br/>工件 + 代码]
    B -->|功能, 添加, 实现| D[部分继承<br/>仅代码]
    B -->|修复, bug, 调整| E[无继承<br/>全新开始]
...
sequenceDiagram
    ...
    participant Tool as HITL 工具
    participant Backend as 交互后端
    ...
    alt CLI 模式
        Backend->>User: 显示预览（终端）
        User->>Backend: 选择选项
        Note over User,Backend: 1. 通过（继续）<br/>2. 编辑（打开 $EDITOR）<br/>3. 反馈（文本输入）
    else GUI 模式
        Backend->>User: 发送 input_request 事件
        User->>Backend: 通过 Tauri 调用响应
        Note over User,Backend: React 模态框带<br/>通过/编辑/反馈选项
    end
    ...
    opt 选择编辑
        Tool->>Tool: 打开外部编辑器
        Tool->>Tool: 通过哈希比较检测更改
    end
    
    Tool-->>Agent: 返回结果
    
    alt 提供反馈
        Agent->>Agent: 带反馈上下文重新生成
        Agent->>Tool: 再次请求审查
    end
```
...
flowchart TB
    subgraph Frontend["React 前端"]
        Iterations[迭代面板] --> Invoke[调用 Tauri 命令]
        App[App 组件] --> Listeners[设置事件监听器]
        Listeners --> Display[实时显示]
    end
    
    subgraph Backend["Tauri 后端"]
        Command[命令处理器] --> Async[生成异步任务]
        Async --> Executor[迭代执行器]
        Executor --> Events[发送事件]
        ProcessRunner[进程运行器] --> Stream[流式日志]
    end
    
    subgraph Core["Cowork 核心"]
        Pipeline[流程] --> Agent[智能体编排]
        Agent --> Tools[工具执行]
        Agent --> LLM[LLM 集成]
    end
...
flowchart LR
    ...
    B -->|项目| C[项目内存<br/>决策和模式]
    B -->|迭代| D[迭代知识<br/>特定见解]
    B -->|智能| E[合并视图<br/>上下文聚合]
...
stateDiagram-v2
    [*] --> Draft: 创建迭代
    Draft --> Running: 开始执行
    Running --> Paused: 用户暂停 / HITL 等待
    Paused --> Running: 继续 / 重试
    Running --> Completed: 所有阶段成功
    Running --> Failed: 错误 / 异常
    
    Completed --> [*]
    Failed --> Running: 带反馈重试
    Failed --> [*]: 中止
```
...
**触发条件**：迭代状态为 `Completed`（Delivery 阶段完成）后，GUI 自动切换到 PM Agent 聊天模式。

**交互模式**：
```
迭代完成 → PM Agent 启用 → 用户自然语言交互 → Agent 意图分析 → 执行操作
```

**支持的操作**：
1. **阶段返回**（goto_stage）：返回到之前阶段重新执行
   - 可选阶段：idea、prd、design、plan、coding
   - 需要用户确认后执行

2. **创建新迭代**（create_iteration）：基于新需求创建新迭代
   - 自动跳转到 plan 阶段开始执行

3. **问答交互**：回答关于项目的问题
   - 提供上下文信息（artifacts、decisions、memory）

**语言偏好**：系统自动读取 Windows 系统语言设置，Agent 生成内容时使用对应语言：
- 中文（zh-CN）：简体中文回复
- 日语（ja）：日本語回复
- 英语（en）：English 回复
- 其他：默认英语
...
**技术实现**：
- 前端：`pmMessages[]` 状态管理聊天历史，`chatMode` 自动切换
- 后端：`pm_send_message` 命令处理消息，`execute_pm_agent_message` 执行 Agent
- 全局配置：`get_system_locale()` 获取系统语言，`get_language_instruction()` 生成语言指令
