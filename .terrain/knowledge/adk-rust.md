...
6. **HITL 工具绕过 InteractiveBackend**：不要直接用 dialoguer/println，必须通过 `InteractiveBackend` trait 与用户交互（CLI/GUI 兼容）
7. **跨平台命令执行**：Windows 用 `cmd /C <cmd>`，Unix 用 `sh -c <cmd>`，用 `cfg!(target_os = "windows")` 分支
8. **子 agent 历史污染**：非 actor-critic 场景用 `IncludeContents::None` 避免上下文干扰
9. **max_iterations 兜底**：始终设置 `with_max_iterations`（建议 2-3），防止 critic 永不调用 exit_loop 导致无限循环
10. **feedback 未消费**：LoopAgent 循环开始前应消费（clear）上轮 feedback，避免重复注入

## LoopAgent vs 顺序 Agent

| 模式 | 何时使用 |
|------|---------|
| `LoopAgent([actor, critic])` | 需要多轮自我审查的产出阶段（PRD/Design/Plan/Coding） |
| 顺序 `vec![agent1, agent2]` (pipeline) | 线性流程，无需迭代反馈 |

## CoworkForge 中的使用模式

- 每个产出阶段（prd/design/plan/coding）使用 `LoopAgent(actor, critic).max_iterations(2)`
- Critic 审查通过 → `exit_loop`；不通过 → `provide_feedback` + 继续循环
- 发现无法解决的问题 → `request_human_review`（escalate=true）
- 用户输入 → `ask_user`（通过 InteractiveBackend）
