...
Cowork Forge 的架构设计遵循“**轻量、可追溯、人机协同、状态即文件**”的核心哲学，旨在构建一个**无需外部依赖、完全本地化、可恢复、可审计**的 AI 原生软件开发引擎。系统摒弃了传统基于数据库的微服务架构，转而采用**基于文件系统的状态持久化模型**，将整个开发流程的中间产物（需求、设计、任务、代码、反馈）以结构化 JSON 文件形式存储于 `.cowork/` 目录中，实现“**状态即代码**”的工程理念。
...
Cowork Forge 是一款面向**软件开发团队**的 AI 原生自动化开发引擎，定位为“**AI 驱动的开发协作者**”，而非替代人类的全自动系统。其核心价值在于：
...
graph LR
    ...
    C --> D[调用IdeaAgent]
    D --> E[调用LLM生成idea文件]
    E --> F[WriteFileTool写入文件]
    F --> G[存储模块更新会话元数据]
    G --> H[调用PRDAgent]
    H --> I[读取idea文件]
    I --> J[调用LLM生成PRD]
    J --> K[CreateRequirementTool创建需求]
    K --> L[保存requirements文件]
    L --> M[ReviewWithFeedbackTool请求审核]
    M --> N[用户编辑反馈]
    N --> O[ProvideFeedbackTool存储反馈]
    O --> P[PRDAgent重新生成]
    P --> Q[更新requirements文件]
    Q --> R[继续后续阶段]
    R --> S[最终生成delivery_report文件]
    S --> T[SaveDeliveryReportTool持久化]
    T --> U[用户查看交付报告]
```
...
> **Cowork Forge 不仅是一个工具，更是一种工程哲学的体现**：  
