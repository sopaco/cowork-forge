...
**描述**：向项目经理智能体发送消息进行交付后交互。PM Agent 可以回答问题、触发阶段跳转或创建新迭代。

**源文件**：`crates/cowork-gui/src-tauri/src/commands/pm.rs`

**参数**：
- `iteration_id`（String）：必需 - 已完成的迭代 ID
- `message`（String）：必需 - 用户发送给 PM Agent 的消息
- `history`（Vec<Value>）：必需 - 对话历史
...
**描述**：从指定阶段重启迭代。在 PM Agent 建议阶段跳转后使用。

**源文件**：`crates/cowork-gui/src-tauri/src/commands/pm.rs`

**参数**：
- `iteration_id`（String）：必需 - 要重启的迭代
- `target_stage`（String）：必需 - 目标阶段（idea、prd、design、plan、coding）

### pm_get_welcome_message

**描述**：获取 PM Agent 界面的初始欢迎消息和操作按钮。
...
```json
{
  "agent_message": "👋 你好！我是项目经理助手...",
  "actions": [
    { "action_type": "pm_start_app", "label": "🚀 启动应用" },
    { "action_type": "pm_view_artifacts", "label": "📄 查看工件" }
  ]
}
```
...
### ExternalAgentConfig 字段

| 字段 | 类型 | 默认值 | 描述 |
|-------|------|---------|-------------|
| `enabled` | bool | false | 启用外部编码代理 |
| `agent_type` | string | "opencode" | 代理标识符 |
| `command` | string | "bun" | 可执行命令 |
| `args` | string[] | ["x", "opencode-ai", "acp"] | 命令参数 |
| `transport` | string | "stdio" | 通信协议 |
| `workspace_path` | string? | null | 覆盖工作区路径 |
| `env` | HashMap? | null | 环境变量 |
...
| 事件 | 方向 | 负载 | 描述 |
|-------|-----------|---------|-------------|
| `pm_message` | 后端 → 前端 | `{ agent_message, actions }` | PM Agent 响应 |
| `pm_actions` | 后端 → 前端 | `{ actions: [...] }` | 可执行按钮 |
| `agent_streaming` | 后端 → 前端 | `{ content, agent_name, is_last }` | 实时文本流 |
| `iteration_created` | 后端 → 前端 | `iteration_id` | 创建新迭代 |
