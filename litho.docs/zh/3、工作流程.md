...
- **回退重跑路径**：从`revert --from <stage>`命令启动，用户可指定任意前置阶段（prd、design、plan、coding、check、delivery）作为新起点，系统创建新 session 并从该阶段重新跑流水线。
- **增量迭代路径**：从`modify <idea>`命令启动，系统以“最近一次成功 session”为基线，先做变更分诊（Change Triage），再做增量代码修改（Code Patch），并生成变更交付报告（Modify Delivery）。
...
- **HITL集成**：在PRD、Design、Plan阶段，智能体在生成草稿内容后，会调用`review_with_feedback_content`（内容模式）工具与用户交互；Idea 阶段使用 `review_and_edit_content`。Coding 阶段仍可通过文件工具读写项目代码文件。
...
- **可追溯**：所有阶段产物与状态都以 session 维度落盘（`idea.md`, `prd.md`, `design.md`, `delivery_report.md` + 各类 JSON state），并在 `.cowork/index.json` 中形成可追溯的执行记录；增量迭代还会生成 `patch/metadata.json` 用于审计代码变更。
