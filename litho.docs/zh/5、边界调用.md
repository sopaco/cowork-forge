...
```bash
cowork new "开发一个在线教育平台"
```

```bash
cowork new "构建一个电商后台管理系统" -c ./myconfig.toml -v
```

```bash
cowork new "创建一个任务管理应用" --stream
```
...
将Cowork Forge作为自动化开发工具集成到CI/CD流水线或开发脚本中，实现一键启动开发流程
...
cowork new "$PROJECT_NAME" --stream --config config.prod.toml
