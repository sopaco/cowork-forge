...
用户交互与入口层是 Cowork Forge V2 系统的唯一对外接口，负责接收用户指令、初始化运行环境，并将控制权安全、准确地移交至核心智能体编排引擎。该层不包含任何业务逻辑，其核心职责是作为**协调者**（Orchestrator）而非**执行者**，确保用户意图能够被精确解析、环境配置被正确加载，并通过标准化接口触发底层自动化开发流程。
...
2. **创建会话服务**：调用 `InMemorySessionService::new()` 创建一个内存会话实例，绑定 `app_name="cowork-forge-v2"` 和 `user_id="cowork-user"`，用于在智能体协作过程中传递上下文。
...
sequenceDiagram
    ...
    CLI->>Session: create_session(app_name="cowork-forge-v2", user_id="cowork-user")
...
2. **`cmd_new` 函数**：
   ...
   async fn cmd_new(description: String, config: &ModelConfig, stream: bool) -> anyhow::Result<()> {
       ...
       let session = InMemorySessionService::new("cowork-forge-v2", "cowork-user");
...
用户交互与入口层是 Cowork Forge V2 系统的“门面”与“守门人”。它以极简的 CLI 接口，实现了复杂 AI 流程的精准触发与环境初始化。其设计不追求炫技，而是以**可靠性、一致性、可预测性**为核心，为上层智能体编排层构建了一个稳定、安全、可信赖的运行环境。正是这一层的严谨设计，使得整个 AI 自动化开发系统能够在无 GUI、无数据库的极端轻量环境下，稳定、可靠地运行，真正实现了“让开发者专注于思考，而非重复劳动”的终极目标。