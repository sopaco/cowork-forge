...
### 5.3 Project Manager Agent Interaction Flow

**Trigger Condition**: After iteration status becomes `Completed` (after Delivery stage), GUI automatically switches to PM Agent chat mode.

**Interaction Pattern**:
```
Iteration Completed → PM Agent Enabled → User Natural Language Interaction → Agent Intent Analysis → Execute Operation
```

**Supported Operations**:
1. **Stage Return** (goto_stage): Return to previous stages for re-execution
   - Available stages: idea, prd, design, plan, coding
   - Requires user confirmation before execution

2. **Create New Iteration** (create_iteration): Create new iteration based on new requirements
   - Automatically jumps to plan stage to start execution

3. **Q&A Interaction**: Answer questions about the project
   - Provides context information (artifacts, decisions, memory)

**Language Preference**: System automatically reads Windows system language setting, Agent generates content using corresponding language:
- Chinese (zh-CN): Simplified Chinese response
- Japanese (ja): 日本語 response
- English (en): English response
- Other: Default English

**Technical Implementation**:
- Frontend: `pmMessages[]` state manages chat history, `chatMode` auto-switches
- Backend: `pm_send_message` command handles messages, `execute_pm_agent_message` executes Agent
- Global config: `get_system_locale()` gets system language, `get_language_instruction()` generates language instruction
