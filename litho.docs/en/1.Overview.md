...
graph TD
    ...
    C[User: Product Manager] -->|Describes idea<br>Approves PRD/Design| B
    D[User: Engineering Lead] -->|Audits sessions<br>Recover pipelines| B

    B -->|Calls via HTTP/JSON<br>Rate-limited| E[OpenAI API<br>(or compatible LLM endpoint)]
    B -->|Reads/Writes session artifacts<br>Session metadata<br>Code files<br>Logs| F[Local File System<br>(.cowork/sessions/)]
    B -->|Opens editor<br>Displays content<br>Collects CLI input| G[User Terminal / Editor<br>(vim, VS Code, etc.)]
    ...
    title C4 System Context Diagram: Cowork Forge
```
