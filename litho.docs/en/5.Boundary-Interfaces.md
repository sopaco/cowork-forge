...
**Source File**: `crates/cowork-gui/src/App.tsx`
...
**Description**: Send a message to the Project Manager Agent for post-delivery interaction. The PM Agent can answer questions, trigger stage jumps, or create new iterations.

**Source File**: `crates/cowork-gui/src-tauri/src/commands/pm.rs`

**Arguments**:
- `iteration_id` (String): required - The completed iteration ID
- `message` (String): required - User's message to the PM Agent
- `history` (Vec<Value>): required - Conversation history

**Response Format**: JSON with streaming events
```json
{
...
**Usage Example**:
```javascript
await invoke('pm_send_message', {
  iterationId: 'iter-1-123',
...
**Description**: Restart an iteration from a specific stage. Used after PM Agent recommends a stage jump.

**Source File**: `crates/cowork-gui/src-tauri/src/commands/pm.rs`

**Arguments**:
- `iteration_id` (String): required - The iteration to restart
- `target_stage` (String): required - Target stage (idea, prd, design, plan, coding)
...
### pm_get_welcome_message

**Description**: Get the initial welcome message and action buttons for the PM Agent interface.
...
```json
{
  "agent_message": "👋 Hello! I'm the Project Manager Agent...",
  "actions": [
    { "action_type": "pm_start_app", "label": "🚀 Start Application" },
    { "action_type": "pm_view_artifacts", "label": "📄 View Artifacts" }
  ]
}
```
...
workspace_path = ""           # optional override
env = { }                     # optional environment variables
```

### ExternalAgentConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | bool | false | Enable external coding agent |
| `agent_type` | string | "opencode" | Agent identifier |
| `command` | string | "bun" | Executable command |
| `args` | string[] | ["x", "opencode-ai", "acp"] | Command arguments |
| `transport` | string | "stdio" | Communication protocol |
| `workspace_path` | string? | null | Override workspace path |
| `env` | HashMap? | null | Environment variables |

---

## PM Agent Events

The following events are emitted during PM Agent interactions:

| Event | Direction | Payload | Description |
|-------|-----------|---------|-------------|
| `pm_message` | Backend → Frontend | `{ agent_message, actions }` | PM Agent response |
| `pm_actions` | Backend → Frontend | `{ actions: [...] }` | Actionable buttons |
| `agent_streaming` | Backend → Frontend | `{ content, agent_name, is_last }` | Real-time text streaming |
| `iteration_created` | Backend → Frontend | `iteration_id` | New iteration created |

---

## Integration Example: PM Agent Workflow

```javascript
// Full PM Agent integration example
import { invoke } from '@tauri-apps/api/core';
import { listen } from '@tauri-apps/api/event';

// Setup event listeners
const unlisten = await listen('agent_streaming', (event) => {
  const { content, is_last, agent_name } = event.payload;
  if (agent_name === 'PM Agent') {
    appendToChat(content);
    if (is_last) finalizeMessage();
  }
});
...
// Get welcome message on PM Agent activation
const welcome = await invoke('pm_get_welcome_message', {
  iterationId: currentIteration.id
});
displayMessage(welcome.agent_message);
renderActionButtons(welcome.actions);
...
async function handleUserMessage(message) {
  ...
  // Actions are in response.actions
...
async function handleAction(action) {
  switch (action.action_type) {
    ...
    case 'pm_create_iteration':
      // New iteration already created, navigate to it
      navigateToIteration(action.iteration_id);
      break;
    case 'pm_start_app':
      await invoke('start_project', { projectId: currentProject.id });
      break;
