...
```mermaid
flowchart TD
    A[User: cowork resume] --> B[CLI: detect .cowork/]
    B --> C[Storage: load_session_metadata()]
    C --> D{Determine last completed stage}
    
    D -->|Idea| E[Start from PRD stage]
    D -->|PRD| F[Start from Design stage]
    D -->|Design| G[Start from Plan stage]
    D -->|Plan| H[Start from Coding stage]
    D -->|Coding| I[Start from Check stage]
    
    E --> J[Storage: load_all_artifacts()]
    F --> J
    G --> J
    H --> J
    I --> J
    
    J --> K[Orchestrator: create_resume_pipeline()]
    K --> L[Execute remaining stages]
