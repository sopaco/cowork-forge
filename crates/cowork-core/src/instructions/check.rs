...
**This is required to save your report, but goto_stage takes priority on failures:**
- You MUST call `save_check_report(content)` when checks PASS
- You MUST call `goto_stage("coding", ...)` when BUILD FAILS (this will save the report internally)
- Without calling these tools, the Check stage CANNOT complete properly
...
项目可以正常运行。
```
然后调用 `save_check_report(content)` 保存报告并完成 Check 阶段。
...
## Step 5: 保存检查报告（MANDATORY）
**这是强制步骤，必须在完成检查后执行！**

无论检查通过或失败，都需要调用 `save_check_report(content)` 保存检查报告：
...
**注意**：如果不调用 `save_check_report`，Check 阶段将无法完成！
...
8. **MANDATORY**: Call save_check_report(content) only when ALL checks PASS
9. **MANDATORY**: Call goto_stage("coding", ...) when ANY build command FAILS
"##
;
