...
const SkillManager: React.FC = () => {
  const { skills, selectedSkill, selectSkill, installSkill, uninstallSkill } =
    useConfigStore();
  ...
  const handleSelectFolder = async () => {
    const selected = await open({
      directory: true,
      multiple: false,
      title: "Select Skill Directory (contains Skill.md)",
    });

    if (selected) {
      await handleInstall(selected as string);
    }
  };
