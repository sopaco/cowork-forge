...
function KnowledgePanel({ currentSession }) {
  ...
  const loadProjectKnowledge = async () => {
    ...
    setLoading(true);
    try {
      const result = await invoke('gui_get_project_knowledge', { projectId: currentSession });
            ...
            {filteredKnowledge.map((knowledge) => (
              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '12px', color: '#666' }}>
                  <Space size="small">
                    <span><ClockCircleOutlined /> {formatDate(knowledge.created_at)}</span>
                  </Space>
                  {knowledge.tech_stack && knowledge.tech_stack.length > 0 && (
                    <Space size="small">
                      {knowledge.tech_stack.slice(0, 2).map((tech, idx) => (
                        <Tag key={`tech-${idx}`} style={{ fontSize: '11px' }}>{tech}</Tag>
                      ))}
                    </Space>
                  )}
                </div>
          ...
          <div style={{ maxHeight: '75vh', overflow: 'auto' }}>
            <Tabs
              ...
              items={[
                {
                  key: 'summary',
                  label: <span><FileTextOutlined /> 总结</span>,
                  children: (
                        ...
                        {selectedKnowledge.idea_summary && (
                          <div style={{ padding: '16px', backgroundColor: '#f6ffed', borderRadius: '8px', border: '1px solid #b7eb8f' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                              ...
                              <Title level={5} style={{ margin: 0, color: '#52c41a' }}>Idea 总结</Title>
                            </div>
                        ...
                        {selectedKnowledge.design_summary && (
                          <div style={{ padding: '16px', backgroundColor: '#f9f0ff', borderRadius: '8px', border: '1px solid #d3adf7' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                              ...
                              <Title level={5} style={{ margin: 0, color: '#722ed1' }}>Design 总结</Title>
                            </div>
                        ...
                        {selectedKnowledge.plan_summary && (
                          <div style={{ padding: '16px', backgroundColor: '#fff7e6', borderRadius: '8px', border: '1px solid #ffd591' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                              ...
                              <Title level={5} style={{ margin: 0, color: '#fa8c16' }}>Plan 总结</Title>
                            </div>
                        ...
                        {selectedKnowledge.code_structure && (
                          <div style={{ padding: '16px', backgroundColor: '#e6f7ff', borderRadius: '8px', border: '1px solid #91d5ff' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                              ...
                              <Title level={5} style={{ margin: 0, color: '#1890ff' }}>代码结构</Title>
                            </div>
                ...
                {
                  key: 'tech-stack',
                  label: <span><CodeOutlined /> 技术栈</span>,
                  children: (
                    <div style={{ padding: '12px 0' }}>
                      <Space direction="vertical" style={{ width: '100%' }} size="middle">
                        {selectedKnowledge.tech_stack?.length > 0 ? (
                          <div style={{ padding: '16px', backgroundColor: '#f5f5f5', borderRadius: '8px' }}>
                            <Text strong style={{ display: 'block', marginBottom: '12px', fontSize: '14px' }}>技术栈</Text>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                              {selectedKnowledge.tech_stack.map((tech, idx) => (
                                <Tag key={`tech-${idx}`} color="blue" style={{ padding: '4px 12px', fontSize: '13px' }}>{tech}</Tag>
                              ))}
                            </div>
                          </div>
                        ) : (
                ...
                {
                  key: 'decisions',
                  label: <span><CheckCircleOutlined /> 关键决策</span>,
                  children: (
                    <div style={{ padding: '12px 0' }}>
                      {selectedKnowledge.key_decisions?.length > 0 ? (
                        <Timeline
                          style={{ marginTop: '12px' }}
                          items={selectedKnowledge.key_decisions.map((decision, idx) => ({
                            children: (
                              <div style={{ padding: '12px', backgroundColor: '#fff', borderRadius: '6px', border: '1px solid #e8e8e8' }}>
                                <Paragraph style={{ margin: 0, lineHeight: '1.8', color: '#333' }}>{decision}</Paragraph>
                              </div>
                            ),
                          }))}
                        />
                ...
                {
                  key: 'patterns',
                  label: <span><BookOutlined /> 设计模式</span>,
                  children: (
                    <div style={{ padding: '12px 0' }}>
                ...
                {
                  key: 'issues',
                  label: <span><CloseCircleOutlined /> 已知问题</span>,
                  children: (
                    <div style={{ padding: '12px 0' }}>
                          ...
                          {selectedKnowledge.known_issues.map((issue, idx) => (
                            <div key={`issue-${idx}`} style={{ padding: '16px', backgroundColor: '#fff2f0', borderRadius: '8px', border: '1px solid #ffccc7' }}>
                              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
                                <CloseCircleOutlined style={{ color: '#ff4d4f', marginTop: '2px' }} />
