...
function App() {
	...
	const handlePMSendMessage = useCallback(async () => {
		console.log(
			'[App] handlePMSendMessage called, userInput:',
			userInput,
			'currentIteration:',
			currentIteration?.id
		);
	...
	const renderContent = () => (
		<div style={{ height: '100%' }}>
			...
			<div
				style={{ height: '100%', display: activeView === 'execution-memory' ? 'block' : 'none' }}
			>
				<MemoryPanel
			...
			<div
				style={{ height: '100%', display: activeView === 'project-knowledge' ? 'block' : 'none' }}
			>
				<KnowledgePanel
			...
			<div style={{ height: '100%', display: activeView === 'chat' ? 'block' : 'none' }}>
				{currentIteration ? (
					<ChatPanel
						...
						onCancelFeedback={() =>
							inputRequest && setInputRequest({ ...inputRequest, isFeedbackMode: false })
						}
